...
// NO-LINT - ZOS Edit - The parameter being int64_t is strang for the internal use in "node-offset"
static int blake2bp_init_leaf( blake2b_state *S, size_t outlen, size_t keylen, uint64_t offset )
{
  ...
  P->depth = 2;
  P->leaf_length = 0;
  P->node_offset = (uint32_t)offset;       // TODO - Don't like this, fix for now, but return to it // NO-LINT - this is C code, no static_cast
  P->xof_length = 0;
  P->node_depth = 0;
